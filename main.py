...
def send_telegram_message(message, token_address):
    ...
    keyboard = {
        "inline_keyboard": [[
            {"text": "[Links] Pump.fun", "url": f"https://pump.fun/{token_address}"},
            {"text": "[Scan] Scamr", "url": f"https://ai.scamr.xyz/token/{token_address}"}
        ], [
            {"text": "[Security] Rugcheck", "url": f"https://rugcheck.xyz/tokens/{token_address}"},
            {"text": "[Mentions] BubbleMaps", "url": f"https://app.bubblemaps.io/sol/token/{token_address}"}
        ], [
            {"text": "[Axiom] Axiom (ref)", "url": f"https://axiom.trade/@glace"},
            {"text": "[AI] Analyze with AI", "url": f"https://pumpfun-bot-1.onrender.com/analyze?token={token_address}"}
        ]]
    }
    payload = {
        "chat_id": CHAT_ID,
...
def check_tokens():
    print("[Scan] Checking tokens...")
    try:
        response = requests.get(API_URL, headers=HEADERS)
        data = response.json().get("result", [])
    except Exception as e:
        print("[ERR] Moralis API error:", e)
        time.sleep(300)
        return
    ...
    for token in data:
        ...
        # [Repeat] 1h follow-up scan for performance messages
        for tracked_token, info in tracking.items():
            ...
            if mc_now > mc_entry and "soar" not in info["alerts"]:
                ...
                if multiplier >= 2:
                    message = f"[Rocket][Rocket][Rocket] ${symbol_tracked} soared by X{multiplier} in an hour since it was called! [Moon]"
                    send_telegram_message(message, tracked_token)
                    info["alerts"].append("soar")

        save_json(tracking, TRACKING_FILE)

        msg = f"""[Scan] *NEW TOKEN DETECTED*

[Token] *Token:* ${symbol}
[Address] *Address:* `{token_address}`

[MarketCap] *Market Cap:* ${int(mc):,}
[Volume] *Volume 1h:* ${int(lq):,}
[Holders] *Holders:* {holders}

[Mentions] *Mentions X*
- Nom: {mentions_name}
- $Ticker: {mentions_ticker}
[Links] [Voir sur X](https://twitter.com/search?q=%24{symbol})

📈 *Bonding Progress:* {bonding_percent or 'N/A'}%
{bonding_bar}

[Security] *Security Check (Rugcheck.xyz)*
- [Burned] Liquidity Burned: [OK]
- [Freeze] Freeze Authority: [OK]
- [Mint] Mint Authority: [OK]
- [Score] Rugscore: {rugscore or 'N/A'} [OK]
- [OK] Token SAFE – LP Locked, No Honeypot

[SmartBuy] *Smart Wallet Buy:* {smart_buy} tokens
...
[Holders] *Top 10 Holders:* {top_total or 'N/A'}%
{top_display}
[Dev] = Dev wallet, [NewWallet] = New wallet (< 2 tokens)
...
"""
        ...
        # [Scan] Wallet deployer history
        if wallet:
            prev_symbol, launch_count, prev_mc = get_wallet_deployment_stats(wallet)
            if prev_symbol:

msg += f"[Deployer] Prev Deployed: ${prev_symbol} ({prev_mc:,})"
[Repeat] # of Launches: {launch_count}"
                if launch_count > 20:
                    msg += " [Serial] Serial Launcher"
                elif launch_count == 1:
                    msg += " [New] First Launch"

        
        # [Mentions] Check if token was already detected earlier and shows new spike
        previous_ts = tracking.get(token_address, {}).get("timestamp")
        if previous_ts and (now - previous_ts > 3600):
            msg += f"\n\n[Repeat] Token previously detected {round((now - previous_ts) / 3600, 1)}h ago – new volume spike!"
        ...
        if previous_ts and (now - previous_ts > 3600):
            ...
            if mc > mc_entry * 2:
                msg += " [Rocket] x2+ pump since first call!"
...
def receive_update():
    ...
    if text == "/scan":
        send_telegram_message("[OK] Scan manuel lancé...", "manual")
        check_tokens()
        ...
        try:
            ...
            alerts = len(tracking)
            msg = f"""[Volume] *Status du bot Pump.fun*

- [Scan] Tokens scannés aujourd'hui : {len(tokens_today)}
- [Rocket] Tokens envoyés depuis lancement : {alerts}"""
...
"
            ...
            msg = "[ERR] Error while retrieving performance data."
        send_telegram_message(msg, "manual")

    elif text == "/help":
        msg = (
            "[AI] *Commandes disponibles*
...
"
            "[Mentions] Smart Wallets • 📈 Bonding Curve • [Security] Rugcheck • [SmartBuy] Whale Tracking • [Holders] Top Holders"
        )
        send_telegram_message(msg, "manual")
...
def analyze_token():
    ...
    send_telegram_message(f"[AI] *GPT Analysis – ${token_data.get('symbol')}*\n\n{result}", token_address)
